...
// List
	function bmf_list( $atts ) {
		...
		$bmf_date       = ( isset( $options ) && isset( $options['date'] ) ) ? $options['date'] : '';
		...
		$bmf_date       = ( isset( $atts['date'] ) ) ? $atts['date'] : $bmf_date;
		// if is allready set up, add shortcode content
		if ( $bmf_api_key && $bmf_agb ) {
			$bmf_url    = BMF_URL;
			$bmf_text   = 'Mitfahrgelegenheiten - Mitfahrzentrale';
			$bmf_config = '{api:"' . $bmf_api_key . '", date:"' . $bmf_date . '", wordpress: "' . get_bloginfo('version') . '"}';
			if ( ! $bmf_place_from && $bmf_place_to ) {
				...
				$bmf_config = '{api:"' . $bmf_api_key . '", date:"' . $bmf_date . '", wordpress: "' . get_bloginfo( 'version' ) . '",to:"' . $bmf_place_to . '"}';
			}
			if ( $bmf_place_from && $bmf_place_to ) {
				...
				$bmf_config = '{api:"' . $bmf_api_key . '", date:"' . $bmf_date . '", wordpress: "' . get_bloginfo( 'version' ) . '",to:"' . $bmf_place_to . '",from:"' . $bmf_place_from . '"}';
			}
			if ( $bmf_place_from && ! $bmf_place_to ) {
				$bmf_url .= '/' . strtolower( urlencode( $bmf_place_from ) ) . '/mitfahrzentrale/';
				$bmf_text   = 'Mitfahrzentrale ' . $bmf_place_from . ' - Mitfahrgelegenheit';
				$bmf_config = '{api:"' . $bmf_api_key . '", date:"' . $bmf_date . '", wordpress: "' . get_bloginfo( 'version' ) . '",from:"' . $bmf_place_from . '"}';
